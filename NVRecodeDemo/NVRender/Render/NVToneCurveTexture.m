...
@implementation NVToneCurveTexture
...
- (instancetype)initWithACVData:(NSData *)data
{
    ...
    if (self) {
        NVACVFile *curve = [[NVACVFile alloc] initWithACVFileData:data];
...
{
    NSArray * rgbCompositeControlPoints = [newValue copy];
    _rgbCompositeCurve = [self getPreparedSplineCurve:rgbCompositeControlPoints];
...
{
    NSArray * redControlPoints = [newValue copy];
    _redCurve = [self getPreparedSplineCurve:redControlPoints];
...
{
    NSArray *  greenControlPoints = [newValue copy];
    _greenCurve = [self getPreparedSplineCurve:greenControlPoints];
...
{
    NSArray * blueControlPoints = [newValue copy];
    _blueCurve = [self getPreparedSplineCurve:blueControlPoints];
...
- (void)dealloc
{
    if (toneCurveByteArray) free(toneCurveByteArray);
}
